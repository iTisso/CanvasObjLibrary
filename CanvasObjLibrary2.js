...
function addEvents(dom,eObj){
	for(var e in eObj){
		dom.addEventListener(e,eObj[e]);
	}
}

//class:CanvasObjectLibrary
function CanvasObjectLibrary() {
	if(!(this instanceof CanvasObjectLibrary)){
		throw(new TypeError("Constructor CanvasObjectLibrary requires 'new'"));
	}
	var COL=this;
	Object.assign({
		/*The main canvas*/
...
Object.assign({
	...
	imageSmoothing:{
		on:function(){this.context.imageSmoothingEnabled = true;},
		off:function(){this.context.imageSmoothingEnabled = false;}
	},
	adjustCanvas:function(width,height){
		this.document.width =this.canvas.width= (this.width = width||this.canvas.offsetWidth);
		this.document.height =this.canvas.height= (this.height = height||this.canvas.offsetHeight);
	}
},CanvasObjectLibrary.prototype);
...
CanvasObjectLibrary.Graph=function(){
	this.useMatrix=false;
	this.style=new CanvasObjectLibrary.styleData();
	this.parentNode=null;
	this.childNode=[];

}
//CanvasObjectLibrary.Graph.prototype.__defineG
Object.assign({
	zoom:function(x, y) {
		if (arguments.length == 1) this.style.zoomX = this.style.zoomY = x;
		else if (arguments.length == 2) {
			this.style.zoomX = x;
			this.style.zoomY = y;
		}
	...
	addEvent:cF.Event.addEvent,
	removeEvent:cF.Event.removeEvent
},CanvasObjectLibrary.Graph.prototype);
CanvasObjectLibrary.styleData=function(){
	this.x=0;
	this.y=0;
	this.zoomX=1;
	this.zoomY=1;
